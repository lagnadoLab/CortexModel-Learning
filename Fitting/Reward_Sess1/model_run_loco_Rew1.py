...
from model_functions_loco_Rew1 import *
...
pc_all = np.loadtxt(r'QA_RewSess1_PC_Avg.txt')
# PC_average_standard_error
pc_all_err = np.loadtxt(r'QA_RewSess1_PC_SEM.txt')

# Experimental time calculation
t_exp_1 = exp_time(start=0.0, step=dt, count=len(pc_all))

# PV_average (whole population)
pv = np.loadtxt(r'QA_RewSess1_PV_Avg.txt')
# PV_average_standard_error
pv_err = np.loadtxt(r'QA_RewSess1_PV_SEM.txt')

# SST_average (whole population)
sst = np.loadtxt(r'QA_RewSess1_SST_Avg.txt')
# SST_average_standard_error
sst_err = np.loadtxt(r'QA_RewSess1_SST_SEM.txt')

# VIP_average (positives and negatives)
vip_pos = np.loadtxt(r'QA_RewSess1_VIPPos_Avg.txt')
vip_neg = np.loadtxt(r'QA_RewSess1_VIPNeg_Avg.txt')

# VIP_average_standard_error
vip_pos_err = np.loadtxt(r'QA_RewSess1_VIPPos_SEM.txt')
vip_neg_err = np.loadtxt(r'QA_RewSess1_VIPNeg_SEM.txt')
input_file = "sobol_samples_Rew1reducedVIPNeg.csv"
...
# Inicialization of new and Adjusting for a missing initial point after deconvolution
pv_new = np.insert(pv, 0, pv[0])
sst_new = np.insert(sst, 0, sst[0])
vip_pos_new = np.insert(vip_pos, 0, vip_pos[0])
vip_neg_new = np.insert(vip_neg, 0, vip_neg[0])
pc_all_new = np.insert(pc_all, 0, pc_all[0])
pv_err_new = np.insert(pv_err, 0, pv_err[0])
sst_err_new = np.insert(sst_err, 0, sst_err[0])
vip_pos_err_new = np.insert(vip_pos_err, 0, vip_pos_err[0])
vip_neg_err_new = np.insert(vip_neg_err, 0, vip_neg_err[0])
pc_all_err_new = np.insert(pc_all_err, 0, pc_all_err[0])

t_exp_1_new = np.append(t_exp_1, t_exp_1[-1] + dt)
###############################################################################

########## VARIABLES AND CONSTANTS ############################################

t = np.float32(np.array(t_exp_1_new))
t_exp = t + 0.107095 #Changed from Hab
...
r_1 = np.float32(1.56756395) #Changed from Hab
delay_1 = np.float32(0.32803978) #Changed from Hab
delay_2 = np.float32(1.95764679) #Changed from Hab
delay_3 = np.float32(0.40521862) #Changed from Hab
decay = np.float32(3.324089)
decay_s = np.float32(0.71415848) #Changed from Hab
decay_f = np.float32(0.89764101) #Changed from Hab
decay_ff = np.float32(2.56867750) #Changed from Hab
ampl = np.float32(1.0)
base = np.float32(1.0)
...
init = np.float32(np.array([0.082, 0.11, 0.16, 0.276, 1.603])) #Neg , 1.000 #Changed from Hab
tau = np.float32(np.array([0.015, 0.0075, 0.019, 0.019]))
i = np.float32(np.array([0.49818416, 0.39958427, 0.39942269, 0.51896040, 1.30012908])) #Neg , 0.94359329 #Changed from Hab
...
w = np.float32(np.array([
              ...
              weights[19],  # w_19 FB  -> VIP_Neg
              weights[20],  # w_20 PC  -> VIP_Neg
              weights[21],  # w_21 SST  -> VIP_Neg
              weights[22],  # w_22 SM -> VIP_Neg
              weights[23]]))  # w_23 VIP_Neg -> SST
...
params.add('w_19', value = w[19] , vary = True, min = 0.0, max = 4) #Neg
params.add('w_20', value = w[20] , vary = True, min = 0.0, max = 4) #Neg 
params.add('w_21', value = w[21] , vary = True, min = 0.0, max = 4) #Neg
params.add('w_22', value = w[22] , vary = True, min = 0.0, max = 4) #Neg
params.add('w_23', value = w[23] , vary = True, min = 0.0, max = 4) #Neg
...
params.add('i_4', value = i[4] , vary = True, min = 0.0, max = 2.7) #Neg
