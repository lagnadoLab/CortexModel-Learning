...
from model_functions_loco_Hab6 import *
...
pc_all = np.loadtxt(r'QA_HabSess6_PC_Avg.txt')
# PC_average_standard_error
pc_all_err = np.loadtxt(r'QA_HabSess6_PC_SEM.txt')

# Experimental time calculation
t_exp_1 = exp_time(start=0.0, step=dt, count=len(pc_all))

# PV_average (whole population)
pv = np.loadtxt(r'QA_HabSess6_PV_Avg.txt')
# PV_average_standard_error
pv_err = np.loadtxt(r'QA_HabSess6_PV_SEM.txt')

# SST_average (whole population)
sst = np.loadtxt(r'QA_HabSess6_SST_Avg.txt')
# SST_average_standard_error
sst_err = np.loadtxt(r'QA_HabSess6_SST_SEM.txt')

# VIP_average (positives and negatives)
vip_pos = np.loadtxt(r'QA_HabSess6_VIPPos_Avg.txt')
vip_neg = np.loadtxt(r'QA_HabSess6_VIPNeg_Avg.txt')

# VIP_average_standard_error
vip_pos_err = np.loadtxt(r'QA_HabSess6_VIPPos_SEM.txt')
vip_neg_err = np.loadtxt(r'QA_HabSess6_VIPNeg_SEM.txt')
input_file = "sobol_samples_Hab6reducedVIPNeg.csv"
...
init = np.float32(np.array([0.055, 0.011, 0.017, 0.208, 1.000])) #Neg 
tau = np.float32(np.array([0.015, 0.0075, 0.019, 0.019]))
i = np.float32(np.array([0.28618393, 0.00729576, 0.09371017, 0.51220849, 0.94359329])) #Neg 
...
w = np.float32(np.array([
              ...
              weights[19],  # w_19 FB  -> VIP_N Neg
              weights[20],  # w_20 PC  -> VIP_N Neg
              weights[21],  # w_21 SST  -> VIP_N Neg
              weights[22],  # w_22 SM -> VIP_N Neg
              weights[23]]))  # w_23 VIP_N -> SST Neg
