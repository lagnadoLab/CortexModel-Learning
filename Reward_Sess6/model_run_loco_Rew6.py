...
from model_functions_loco_Rew6 import *
...
pc_all = np.loadtxt(r'QA_RewSess6_PC_Avg.txt')
# PC_average_standard_error
pc_all_err = np.loadtxt(r'QA_RewSess6_PC_SEM.txt')

# Experimental time calculation
t_exp_1 = exp_time(start=0.0, step=dt, count=len(pc_all))

# PV_average (whole population)
pv = np.loadtxt(r'QA_RewSess6_PV_Avg.txt')
# PV_average_standard_error
pv_err = np.loadtxt(r'QA_RewSess6_PV_SEM.txt')

# SST_average (whole population)
sst = np.loadtxt(r'QA_RewSess6_SST_Avg.txt')
# SST_average_standard_error
sst_err = np.loadtxt(r'QA_RewSess6_SST_SEM.txt')

# VIP_average (positives and negatives)
vip_pos = np.loadtxt(r'QA_RewSess6_VIPPos_Avg.txt')
vip_neg = np.loadtxt(r'QA_RewSess6_VIPNeg_Avg.txt')

# VIP_average_standard_error
vip_pos_err = np.loadtxt(r'QA_RewSess6_VIPPos_SEM.txt')
vip_neg_err = np.loadtxt(r'QA_RewSess6_VIPNeg_SEM.txt')
input_file = "sobol_samples_Rew6reducedVIPNeg.csv"
...
delay_1 = np.float32(0.33452149) #Changed from Rew1
delay_2 = np.float32(1.95764679) #Changed from Hab
delay_3 = np.float32(0.40598527) #Changed from Hab
decay = np.float32(3.324089)
decay_s = np.float32(0.71080014) #Changed from Hab
decay_f = np.float32(0.91210523) #Changed from Hab
decay_ff = np.float32(2.58516911) #Changed from Hab
ampl = np.float32(1.0)
base = np.float32(1.0)
...
init = np.float32(np.array([0.105, 0.15, 0.27, 0.42, 1.16])) #Neg  #Changed from Rew1
tau = np.float32(np.array([0.015, 0.0075, 0.019, 0.019]))
i = np.float32(np.array([1.2, 1.5, 0.6, 0.7, 1.3])) #Neg , 0.94359329 #Changed from Rew1
...
params = Parameters()
params.add('w_0', value = w[0] , vary = True, min = 0.00, max = 1.0)
